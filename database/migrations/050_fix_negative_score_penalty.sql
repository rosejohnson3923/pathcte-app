-- ============================================
-- FIX: Prevent negative scores from penalties
-- ============================================
-- When applying incorrect answer or no answer penalties,
-- ensure the player's score never goes below 0.
...
BEGIN
  ...
  v_new_score := GREATEST(0, v_player.score + v_points_earned);
...
'Securely submit an answer and update player score. Uses configurable scoring from game_mode_scoring table. Ensures score never goes below 0. Runs with elevated privileges and bypasses player validation trigger.';

-- ============================================
-- ALSO FIX: apply_no_answer_penalty
-- ============================================
-- This function also needs to prevent negative scores
-- ============================================
...
BEGIN
  ...
  LOOP
    -- Calculate new score and ensure it doesn't go below 0
    v_new_score := GREATEST(0, v_player.score + v_penalty);
...
'Applies configurable penalty to all players who did not submit an answer before timeout. Ensures score never goes below 0. Works for all game modes. Set penalty to 0 in game_mode_scoring to disable. Called when advancing to the next question in auto mode.';
