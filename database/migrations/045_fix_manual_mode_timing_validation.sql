-- ============================================
-- Fix timing validation for manual progression mode
-- ============================================
-- In manual mode, students should be able to answer until
-- the host advances to the next question, regardless of timer
-- ============================================
...
BEGIN
  ...
  -- Get game session to check progression control
  ...
  -- Always check minimum time (prevent cheating)
  IF p_time_taken_ms < 100 THEN
    RAISE EXCEPTION 'Invalid timing: answer too fast (minimum 100ms)';
  END IF;

  -- Only enforce "too slow" validation in AUTO mode
  -- In MANUAL mode, students can answer until host advances
  -- Extract progression_control from settings JSONB (defaults to 'manual' if not set)
  IF COALESCE(v_session.settings->>'progressionControl', 'manual') = 'auto'
     AND p_time_taken_ms > v_time_limit_ms + 5000 THEN
  ...
  IF v_is_correct THEN
    ...
    -- Only award speed bonus if answer is within time limit
...
'Securely submit an answer and update player score. In manual mode, timing validation is relaxed to allow answers until host advances. In auto mode, strict timing is enforced.';
