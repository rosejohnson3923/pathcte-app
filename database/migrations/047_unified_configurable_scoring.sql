-- ============================================
-- UNIFIED CONFIGURABLE SCORING SYSTEM
-- ============================================
-- Replaces mode-specific hardcoded scoring with a configurable
-- system that works for all game modes. Each mode can have
-- different point values that can be changed independently.
-- Setting penalties to 0 disables them.
-- ============================================

-- Create scoring configuration table
CREATE TABLE IF NOT EXISTS game_mode_scoring (
  game_mode TEXT PRIMARY KEY,
  correct_points INTEGER NOT NULL DEFAULT 25,
  incorrect_penalty INTEGER NOT NULL DEFAULT -10,
  no_answer_penalty INTEGER NOT NULL DEFAULT -10,
  updated_at TIMESTAMPTZ DEFAULT NOW()
);

-- Insert default values for all game modes (25/-10/-10)
INSERT INTO game_mode_scoring (game_mode, correct_points, incorrect_penalty, no_answer_penalty)
VALUES
  ('career_quest', 25, -10, -10),
  ('path_defense', 25, -10, -10),
  ('career_clash', 25, -10, -10),
  ('mystery_path', 25, -10, -10),
  ('speed_run', 25, -10, -10),
  ('team_challenge', 25, -10, -10)
ON CONFLICT (game_mode) DO NOTHING;

-- Add comment
COMMENT ON TABLE game_mode_scoring IS
'Configurable scoring values per game mode. Set penalties to 0 to disable them.';

-- Grant access
GRANT SELECT ON game_mode_scoring TO authenticated;

-- ============================================
-- UPDATE: submit_answer_securely
-- ============================================
-- Now reads scoring from configuration table instead of
-- hardcoded Lightning! Path-specific values
-- ============================================
...
-- ============================================
-- UPDATE: apply_no_answer_penalty
-- ============================================
-- Now reads penalty from configuration table and works
-- for all game modes (not just Lightning! Path)
-- ============================================
...
'Securely submit an answer and update player score. Uses configurable scoring from game_mode_scoring table. Runs with elevated privileges to bypass RLS restrictions while maintaining security.';
